...
class BookingsAdapter(private val context: Context, private val bookings: List<HashMap<String, Any>>) : RecyclerView.Adapter<BookingsAdapter.BookingViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BookingViewHolder, position: Int) {
        ...
        holder.bookingIdTextView.text = "Номер бронирования: ${booking["BookingID"]}"
        //holder.orderIdTextView.text = "Order ID: ${booking["OrderID"]}"
        holder.pickupLocationTextView.text = "Точка отправления: ${booking["PickupLocation"]}"
        holder.dropoffLocationTextView.text = "Точка прибытия: ${booking["DropoffLocation"]}"
        holder.statusTextView.text = "Статус: ${booking["Status"]}"
